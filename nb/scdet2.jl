...
# ╔═╡ f80f1305-77a2-4f51-8f4d-b6d7b7433f77
begin
	using Geant4
...
# ╔═╡ 81a6d8eb-1b84-4c6b-9b69-66b84eb899a8
begin
	import PlutoUI
	using FHist
	using Printf
...
# ╔═╡ fd14bbf7-3581-4047-a1af-ede17d11858e
...
# ╔═╡ 6c47c5cf-5c48-4b93-8047-3f626fbd6f59
jp = ingredients("../src/JUPETER.jl")

# ╔═╡ eab24142-450d-4b15-a3fe-66b0e4bcad6e
jp.JUPETER.hello()

# ╔═╡ 5dfda70b-7fc0-4992-b101-cc3aa00fd2e5
crysdet = jp.JUPETER.ScintDetector()

# ╔═╡ 13559ea3-eba4-4564-89a6-a1e6d1f9e6cd
...
# ╔═╡ ab30fe92-d222-4232-a8d6-7b20a2cafa9f
a = randn(10000);

# ╔═╡ bac9a365-364f-45fe-9744-5aa66bae41b6
h1 = Hist1D(a, -4:0.1:4)

# ╔═╡ 263f72f6-f94c-4ef4-9c03-a1955389398d
fieldtypes(typeof(h1))

# ╔═╡ fc7079eb-e7e9-49c0-abb8-52220b520a94
h1.hist

# ╔═╡ 03e6d954-b80c-4258-8e68-a308d00ae3ea
nentries(h1)

# ╔═╡ 7fbb1307-6f0f-4aee-8486-cf8bb22690cb
h1.hist.weights

# ╔═╡ e988ea6c-c714-4c9b-9b15-3184da73a90d
typeof(h1)

# ╔═╡ 4d54369c-13eb-4ac2-b00a-21540f9f7175
nbins(h1)

# ╔═╡ de297f50-9ed4-49ef-9f40-2787d558c10c
integral(h1)

# ╔═╡ 4642a3ac-92a6-485a-942b-bd195251f6fb
bincenters(h1)

# ╔═╡ bde45a53-821c-417c-9bd0-7fb54cf50bf1
plot(h1)

# ╔═╡ 81383543-4db4-4ccb-a131-20ecfd79e75f
const Hist1D64 = Hist1D{Float64, Tuple{StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}}}


# ╔═╡ 98375f83-62be-45fd-a202-1115e9aaaa81
"""
...
# ╔═╡ 637e23f0-53da-11ee-1c6e-edbf29cbd9c0
using Pkg; Pkg.activate(findrdir())

# ╔═╡ 3b362e55-2836-42a3-8202-ce2b6a824105
function histxy(h1d::Hist1D)
	bincenters(h1d), h1d.hist.weights
end

# ╔═╡ 4d3a5f48-8147-4912-b2b0-3f5f2185512a
function ppl(h1)
	plot(histxy(h1)..., seriestype=:bar)
end

# ╔═╡ f9c75a2a-4217-4e8a-b278-97b3cc7fed95
ppl(h1)

# ╔═╡ 22e89b06-ae96-4905-8a55-5be2c3bc42a6
...
# ╔═╡ e52200a4-05e3-4f54-9afc-a6e123e63ad3
"""
Adds ScintSimData
...
# ╔═╡ 2b1d98ed-680d-4d55-9de7-c3a4e39b8545
"""
...
function do_plot(data::ScintSimData)
    ...
    plot!(subplot=1, histxy(scintPhotonsHisto)..., seriestype=:bar,title="# scintillating photons/event", 
		  xlabel="# photons", show=true)
    plot!(subplot=2, histxy(siHitsHisto)..., seriestype=:bar, title="# hits in Silicon/event", 
		  xlabel="# hits")
    plot!(subplot=3, histxy(crysEdepHisto)..., seriestype=:bar, title="energy deposited in crystal", 
		  xlabel="keV")
end

# ╔═╡ d5b00da5-05a4-47a6-a0e9-4b703c1b44ca
"""
Define physics list
"""
...
# ╔═╡ 6428a09d-47df-4d2c-91c3-7787e50f2c5c
"""
...
# ╔═╡ 0d952c69-a215-4018-a1af-4bce3a85d247
@enum HitType Si ScintCryst

# ╔═╡ 9da2ac12-e6c7-4a80-9fa8-e1e4e363050e
"""
Defines a hit:
...
# ╔═╡ 7c199a22-72d4-4a1b-bf34-f743cefc13b0
"""
...
# ╔═╡ 63f1aa04-7492-4ea2-89f3-41ad2ac4e7ba
"""
...
# ╔═╡ bb381633-ff73-4eba-b034-cb4aa44cecb6
"""
...
# ╔═╡ 9076c2a9-d649-4bb5-bb58-aa96f60729bc
...
# ╔═╡ bb56bb7a-9780-43f0-9df1-d6ebd0c1d364
crystal_SD = jp.JUPETER.G4JLSensitiveDetector("Crystal_SD", 
...
# ╔═╡ 61def3b2-6a32-43fc-9e9b-137c1ac2a54f
"""
...
# ╔═╡ 181bc6e1-ef80-4f13-a896-8ef3eb5d7878
"""
...
# ╔═╡ a635640f-634b-4d16-bbd8-f2068a659d5e
silicon_SD = jp.JUPETER.G4JLSensitiveDetector("Silicon_SD", 
	                                SiSDData();               
                                    processhits_method=si_processHits,     
                                    initialize_method=si_initialize)    

# ╔═╡ be8dcf06-54f0-4b64-be49-4d261bafc33e
"""
...
# ╔═╡ 909df03c-bdfc-494d-8617-cfef6a3e6b98
"""
...
# ╔═╡ cefc2231-32e9-424a-846f-5271b8c367f6
"""
...
# ╔═╡ a5257152-e7a8-46a4-a064-4bb768076df6
"""
...
# ╔═╡ 1010cf22-77e1-404d-ad82-69a5aa873d7f
"""
End Event Action
...
# ╔═╡ cba6c650-d7e8-4aa1-932d-6a123fc4aee9
"""
...
# ╔═╡ 69c28b79-0408-45b2-b9a7-bde4b5fff6c4
let G4OpticalPhoton, first=true
...
# ╔═╡ 5fcfe879-2030-4c73-9cc7-e8b4c4ffc4fe
...
# ╔═╡ 51d35cb0-fc61-4883-adea-844e2180ab70
configure(app)

# ╔═╡ 44d8e1b5-8e12-4c08-9b72-742370315ac5
initialize(app)

# ╔═╡ 157064d1-dccb-401d-b13a-277290c4569c
beamOn(app,1000)

# ╔═╡ 5d0eb50c-87e9-4fe6-a8cb-d4d858028b32
do_plot(app.simdata[1])

# ╔═╡ 398112d6-6c6e-4403-b38b-1478ed75975c
(;scintPhotonsHisto, siHitsHisto, crysEdepHisto) = app.simdata[1]

# ╔═╡ 6f0f8091-c2ff-46e5-87b2-b930764fe5e8
scintPhotonsHisto

# ╔═╡ ea772ebc-3c45-4725-b8fd-7dbd14d28189
typeof(scintPhotonsHisto)

# ╔═╡ 18d73fb8-c44d-4a43-bde8-32f60a4db350
siHitsHisto

# ╔═╡ d07dda1b-dd4a-4260-befd-1a5c10412d24
crysEdepHisto

# ╔═╡ 7181cc14-a752-40b3-bd04-341d5b31ad6e
histxy(scintPhotonsHisto)

# ╔═╡ 03426cc3-415f-42fb-ab05-1a0eadee9710
ppl(scintPhotonsHisto)

# ╔═╡ 74105d98-1ae2-4581-bd01-14cb7a6d1d9a
begin
...
# ╔═╡ d305d174-bf10-4861-9bf2-424e05876dd9
"""
...
# ╔═╡ e06085ee-84a7-4ff7-bc05-f7934dc43b5d
"""
